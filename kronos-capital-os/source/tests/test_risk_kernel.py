...
def test_clips_size():
    cfg, a, i = fixtures()
    d = RiskKernel(cfg).evaluate(i, a, datetime.now(timezone.utc))
    assert d.approved and d.approved_qty < i.qty


def test_stale_blocks():
    cfg, a, i = fixtures()
    d = RiskKernel(cfg).evaluate(
        i, a, datetime.now(timezone.utc) - timedelta(seconds=7)
    )
...
def test_emergency_blocks():
    cfg, a, i = fixtures()
    d = RiskKernel(cfg).evaluate(i, a, datetime.now(timezone.utc), True)
    assert not d.approved
