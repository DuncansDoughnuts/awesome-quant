...
class DatabentoFeed:
    ...
    async def health(self):
        return (
            ConnectorState.CONNECTED if self.connected else ConnectorState.RECONNECTING
        )
        ...
        def sync():
            ...
            for rec in c:
                ...
                if px > 1e8:
                    px /= 1e9
                symbol = str(
                    getattr(rec, "symbol", getattr(rec, "instrument_id", "UNKNOWN"))
                )
                asyncio.run_coroutine_threadsafe(
                    on_event(MarketEvent("DATABENTO", symbol, "UNKNOWN", px)), loop
                ).result()
