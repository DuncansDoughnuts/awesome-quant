...
class CoinbaseConnector:
    ...
    async def account_state(self):
        ...
        for a in getattr(accounts, "accounts", []) or []:
            if getattr(a, "currency", "") in ("USD", "USDC"):
                cash += float(
                    getattr(getattr(a, "available_balance", None), "value", 0) or 0
                )
    ...
    async def place_order(self, intent, approved_qty):
        return self.client.market_order(
            client_order_id=intent.metadata.get("client_order_id", intent.strategy_id),
            product_id=intent.instrument,
            side=intent.side,
            base_size=str(approved_qty),
        )
