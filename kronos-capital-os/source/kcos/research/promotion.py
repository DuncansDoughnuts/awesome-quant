...
class PromotionEngine:
    ...
    def next_stage(self, current, metrics):
        ...
        if (
            ...
            and metrics.get("leakage_flags", 0) == 0
            and metrics.get("oos_observations", 0) >= 100
        ):
            return StrategyStage.WALK_FORWARD
        if (
            c == StrategyStage.WALK_FORWARD
            and metrics.get("positive_oos_windows", 0) >= 3
            and metrics.get("cost_model", False)
        ):
            return StrategyStage.PAPER
        if (
            c == StrategyStage.PAPER
            and metrics.get("paper_trades", 0) >= 50
            and metrics.get("paper_days", 0) >= 14
            and metrics.get("paper_drawdown", 1) < 0.08
        ):
            return StrategyStage.CANARY
        if (
            c == StrategyStage.CANARY
            and metrics.get("canary_trades", 0) >= 30
            and metrics.get("canary_drawdown", 1) < 0.05
        ):
            return StrategyStage.LIVE
        if (
            c == StrategyStage.LIVE
            and metrics.get("live_trades", 0) >= 100
            and metrics.get("net_expectancy", 0) > 0
        ):
