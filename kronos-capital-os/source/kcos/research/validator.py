...
def validate_returns(returns, min_observations=100, assumed_cost_per_period=0.0):
    ...
    dd = (peaks - curve) / np.maximum(peaks, 1e-12)
    sd = x.std(ddof=1)
    sharpe = float(x.mean() / sd * np.sqrt(252)) if sd > 0 else 0
    ...
    return ValidationResult(
        ...
        float(x.mean()),
        sharpe,
        float(dd.max(initial=0)),
        "passed" if passed else "gate_failed",
    )
